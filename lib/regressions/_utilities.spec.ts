...
describe('Utilities', () => {
    ...
    it('Predict Multivariable Function', async () => {
        ...
        ).toEqual([3.9000000000000004, 4.550000000000001])
    })
    ...
    it('Transform to Polynomial Input', async () => {
        expect(transformToPolynomialInput([1, 2], 2)).toEqual([
            [1, 1],
            [2, 4]
        ])
    })
    ...
    it('Decision Boundary', async () => {
        expect(decisionBoundary([0.9, 0.2, 0.5])).toEqual([1, 0, 1])
    })
})
